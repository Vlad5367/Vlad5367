...
def show_recipe_window(recipe_details):
    ...
    ingredients_label.grid(row=0, column=0, padx=10, pady=10)
    ingredients_text = ctk.CTkTextbox(recipe_window, width=80, height=10)
    ingredients_text.insert('end', recipe_details[2])
    ingredients_text.grid(row=0, column=1, padx=10, pady=10, columnspan=2)
    instructions_label = ctk.CTkLabel(recipe_window, text="Инструкции:")
    instructions_label.grid(row=1, column=0, padx=10, pady=10)
    instructions_text = ctk.CTkTextbox(recipe_window, width=80, height=10)
    instructions_text.insert('end', recipe_details[3])
    instructions_text.grid(row=1, column=1, padx=10, pady=10, columnspan=2)
...
title_entry.grid(row=0, column=1, padx=10, pady=10, columnspan=2)
ingredients_label = ctk.CTkLabel(root, text="Ингредиенты:")
ingredients_label.grid(row=1, column=0, padx=10, pady=10)
ingredients_entry = ctk.CTkTextbox(root, width=80, height=10)
ingredients_entry.grid(row=1, column=1, padx=10, pady=10, columnspan=2)
instructions_label = ctk.CTkLabel(root, text="Инструкции:")
instructions_label.grid(row=2, column=0, padx=10, pady=10)
instructions_entry = ctk.CTkTextbox(root, width=80, height=10)
instructions_entry.grid(row=2, column=1, padx=10, pady=10, columnspan=2)
add_button = ctk.CTkButton(root, text="Добавить рецепт", command=add_recipe)
add_button.grid(row=3, column=0, columnspan=3, padx=10, pady=10)
listbox = CTkListbox(root, command=show_recipe, width=80, height=20)
listbox.grid(row=4, column=0, columnspan=3, padx=10, pady=10, sticky="nsew")
